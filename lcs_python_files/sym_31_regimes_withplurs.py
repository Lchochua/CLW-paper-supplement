...
def Pref(P,S,X,k,m,n):
    ...
    for p in P:
        ...
        entry = 0
        # Utilities for MFN
        Util[pme][entry] = matrices_dict[f"MFN_{n}"][k,m] + matrices_dict["Tr_MFN"][k,m]
        ...
        for v in range(2):
            ...
            Util[pime][entry] = matrices_dict[f"CUI_{n}"][k,m] + matrices_dict["Tr_CUI"][k,m]
        # CU outsider
        pome=((p) % 3)
        #pio1=((p+1) % 3)
        #pio2=((p+2) % 3)
        ...
        Util[pome][entry] = matrices_dict[f"CUO_{n}"][k,m] + matrices_dict["Tr_CUO"][k,m] 
        ...
        for v in range(2):
            ...
            Util[pime][entry] = matrices_dict[f"FTAI_{n}"][k,m] + matrices_dict["Tr_FTAI"][k,m]
        ...
        Util[pome][entry] = matrices_dict[f"FTAO_{n}"][k,m] + matrices_dict["Tr_FTAO"][k,m] 
        ...
        Util[pime][entry] = matrices_dict[f"HUB_{n}"][k,m] + matrices_dict["Tr_HUB"][k,m] 
        ...
        for v in range(2):
            ...
            Util[pome][entry] = matrices_dict[f"SPOKE_{n}"][k,m] + matrices_dict["Tr_SPOKE"][k,m]
        ...
        for v in range(2):
            ...
            #poo=((p+1+((v+1) % 2)) % 3)
            entry=(12+((p+2*v) % 3))             
            # Utilities for the Insider of PL(pime,pio)            
            Util[pime][entry] = matrices_dict[f"PLI_{n}"][k,m] + matrices_dict["Tr_MFN"][k,m]
        # Subcase .3: Outsider of an PL 
        pome=((p) % 3)
        #pio1=((p+1) % 3)
        #pio2=((p+2) % 3)
        entry=(12+((p+1) % 3))
        # Utilities for the Outsider of PL(pio1,pio2)
        Util[pome][entry] = matrices_dict[f"PLO_{n}"][k,m] + matrices_dict["Tr_MFN"][k,m]
        # Case 15: GPL (plurilateral among all three countries, MFN in traditional sectors) 
        pme = ((p) % 3)
        entry = 15
        # Utilities for three country plurilateral agreement
        Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_MFN"][k,m]
        
        # Case: 16-21 (difficult to assign single function to insider-outsider (no clear division). We do individual cases)
        # Case 16
        pme = ((p) % 3)
        entry = 16
        if p == 0:
            Util[pme][entry] = matrices_dict[f"aFTAPL_{n}"][k,m] + matrices_dict["Tr_FTAI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"bFTAPL_{n}"][k,m] + matrices_dict["Tr_FTAI"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"cFTAPL_{n}"][k,m] + matrices_dict["Tr_FTAO"][k,m]
        # Case 17
        pme = ((p) % 3)
        entry = 17
        if p == 0:
        ...
        # Cases: 22-24
        # Insider of FTAGPL (two-country FTA and three-country PL)
        for v in range(2):
            pime=((p) % 3)
            entry=(22+((p+2*v) % 3))             
            # Utilities for the Insider of FTA in FTAPL           
            Util[pime][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTAI"][k,m]
        # Subcase .3: Outsider of FTA
        pome=((p) % 3)
        #pio1=((p+1) % 3)
        #pio2=((p+2) % 3)
        entry=(22+((p+1) % 3))
        # Utilities for the Outsider of FTA in FTAPL
        Util[pome][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTAO"][k,m]
        
        # Cases: 25-27
        # Regime generates global free trade in non-traditional sector. 
        # The only difference comes from traditional sector - whether country is hub or spoke
        # Subcase .1: Hub of an FTAHub             
        pime=((p) % 3)
        #poo1=((p+1) % 3)
        #poo2=((p+2) % 3)
        entry=(25+((p) % 3)) 
        # Utilities for the Hub of FTAHub(pime)  
        Util[pime][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB"][k,m] 
        # Subcases .2-3: Part (not Hub) of an FTAHub
        # Auxiliaries for the Parts (not Hub) of an FTAHub         
        for v in range(2):
            pome=((p) % 3)
            ...
            entry=(25+((p+1+v) % 3)) 
            # Utilities for the Parts (not Hub) of FTAHub(pio)
            Util[pome][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_SPOKE"][k,m]
            
        # Case 28 CUPL(12,123)
        pme = ((p) % 3)
        entry = 28
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CUI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CUI"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CUO"][k,m]
        ...
        Util[pme][entryCU] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_GFT"][k,m] 
        # Utilities for GFTFTA [Case 11]        
        Util[pme][entryFTA] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_GFT"][k,m] 
...
def main():
    ...
    df.to_excel(f'sym_stable_sets_31_regimes_full_{n}.xlsx', index=False)
    print('The End')
