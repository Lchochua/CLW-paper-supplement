...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 10:20:05 2023
...
n = 25 # Choose the number of firms in a non-traditional sector
...
def Pref(P,S,X,k,m,n):
    ...
    for p in P:
        ...
        entry = 1
        if p == 0:
            Util[pme][entry] = matrices_dict[f"CUI_{n}"][k,m] + matrices_dict["Tr_CU_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"CUI_{n}"][k,m] + matrices_dict["Tr_CU_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"CUO_{n}"][k,m] + matrices_dict["Tr_CU_sl_LO"][k,m]
        ...
        entry = 3
        if p == 0:
            Util[pme][entry] = matrices_dict[f"CUI_{n}"][k,m] + matrices_dict["Tr_CU_ll_L"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"CUO_{n}"][k,m] + matrices_dict["Tr_CU_ll_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"CUI_{n}"][k,m] + matrices_dict["Tr_CU_ll_L"][k,m]
        ...
        entry = 5
        if p == 0:
            Util[pme][entry] = matrices_dict[f"FTAI_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"FTAI_{n}"][k,m] + matrices_dict["Tr_FTA_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"FTAO_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LO"][k,m]
        ...
        entry = 7
        if p == 0:
            Util[pme][entry] = matrices_dict[f"FTAI_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"FTAO_{n}"][k,m] + matrices_dict["Tr_FTA_ll_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"FTAI_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]
        ...
        entry = 8
        if p == 0:
            Util[pme][entry] = matrices_dict[f"HUB_{n}"][k,m] + matrices_dict["Tr_HUB_l_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"SPOKE_{n}"][k,m] + matrices_dict["Tr_HUB_l_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"SPOKE_{n}"][k,m] + matrices_dict["Tr_HUB_l_LO"][k,m]
        ...
        if p == 0:
            ...
            Util[pme][entry] = matrices_dict[f"HUB_{n}"][k,m] + matrices_dict["Tr_HUB_s_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"SPOKE_{n}"][k,m] + matrices_dict["Tr_HUB_s_L"][k,m]
        ...
        entry = 16
        if p == 0:
            Util[pme][entry] = matrices_dict[f"aFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"bFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"cFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LO"][k,m]
            
        # Case 17 FTAPL(12,23)
        pme = ((p) % 3)
        entry = 17
        if p == 0:
            Util[pme][entry] = matrices_dict[f"bFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"aFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"cFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LO"][k,m]
        ...
        entry = 20
        if p == 0:
            Util[pme][entry] = matrices_dict[f"aFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"cFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_ll_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"bFTAPL_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]
        ...
        entry = 22
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_sl_LO"][k,m]
        ...
        entry = 24
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_ll_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_FTA_ll_L"][k,m]   
        ...
        entry = 25
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB_l_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB_l_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB_l_LO"][k,m]
        ...
        if p == 0:
            ...
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB_s_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_HUB_s_L"][k,m]
        ...
        entry = 28
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_sl_LI"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_sl_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_sl_LO"][k,m]
        ...
        entry = 30
        if p == 0:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_ll_L"][k,m]
        elif p == 1:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_ll_S"][k,m]
        else:
            Util[pme][entry] = matrices_dict[f"GFT_{n}"][k,m] + matrices_dict["Tr_CU_ll_L"][k,m]   
...
def main():
    ...
    df.to_excel(f'sll_stable_sets_31_regimes_full_{n}.xlsx', index=False)
    print('The End')
...
# # =============================================================================
# # # Define the lists for sigma and alpha values
# sigma_values = [95,95,95]  
# alpha_values = [10,225,400] 


# # Define the subdirectory for saving files
# ranking_dominance_directory = os.path.join(directory_path, f"N_is_{n}_analysis_utilities_rankings")


# # Ensure the subdirectory exists (create it if necessary)
# os.makedirs(ranking_dominance_directory, exist_ok=True)

# # Iterate through each pair of sigma and alpha using zip
# for sigma, alpha in zip(sigma_values, alpha_values):
#     # Perform the preference operation
#     M = Pref(P, S, range(nmax), sigma, alpha, n)
    
#     # Extract utility and ranking from M
#     utility, ranking = M[0], M[1]
    
#     # Convert utility to a DataFrame and save to Excel
#     utility_df = pd.DataFrame(np.array(utility))
#     utility_filename = f"{sigma}_{alpha}_sll_utility_31_regimes_{n}_firms.xlsx"
    
#     # Define the full file path for the utility file
#     utility_filepath = os.path.join(ranking_dominance_directory, utility_filename)
    
#     # Save the utility DataFrame to an Excel file
#     utility_df.to_excel(utility_filepath, index=True)
    
    

#     # Iterate through each coalition to save ranking matrices
#     for coalition in range(7):
#         ranking_matrix = ranking[coalition]
#         ranking_df = pd.DataFrame(ranking_matrix)
#         ranking_filename = f"{sigma}_{alpha}_sll_{coalition}_ranking_31_regimes_{n}_firms.xlsx"
        
#         # Define the full file path for the ranking file
#         ranking_filepath = os.path.join(ranking_dominance_directory, ranking_filename)
        
#         # Save the ranking DataFrame to an Excel file
#         ranking_df.to_excel(ranking_filepath, index=True)



# #================================================================================================
# # Define the target directory for saving results
# ind_dominance_directory = os.path.join(directory_path, f"N_is_{n}__analysis_ind_dominance")

# # Create the directory if it does not exist
# os.makedirs(ind_dominance_directory, exist_ok=True)

# # Iterate through each pair of sigma and alpha using zip
# for sigma, alpha in zip(sigma_values, alpha_values):
#     # Perform the search operation
#     LCS = Search(S, range(nmax), Network(nmax, S), Pref(P, S, range(nmax), sigma, alpha, n)[1], 0)
#     ind_dom = LCS[1]

#     # Create a DataFrame from the result
#     df = pd.DataFrame(ind_dom)

#     # Define the filename using the current sigma and alpha values
#     filename = f"{sigma}_{alpha}_sll_ind_dom_31_regimes_{n}_firms.xlsx"

#     # Join the directory path with the filename
#     dom_file_path = os.path.join(ind_dominance_directory, filename)

#     # Save the DataFrame to an Excel file in the target directory
#     df.to_excel(dom_file_path, index=True)
